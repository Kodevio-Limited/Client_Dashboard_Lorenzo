...
function PropertyMediaContent() {
  ...
  const { data: allMedia, isLoading } = useMediaQuery();

  const propertyMedia = allMedia?.filter((m) =>
    propertyId ? String(m.propertyId) === String(propertyId) : true
  );
        ...
        <div className="bg-dark-600 rounded-[8px] p-6 max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {propertyMedia.map((item, idx) => {
              ...
              const title = item.title || (isVideo ? 'Video Tour' : 'Property Photo');
...
export default function PropertyMediaPage() {
  return (
    <>
      <Header />
      <Suspense fallback={<div className="p-8 text-white">Loading media...</div>}>
